####################################
#John Lewis October 2013
#Beginings of code to run correct mcmc algorithm when conditing on robust statistics
#Focus here is on Tukey's loss and Huber's proposal 2 to form the estimating equations for the location and scale in the location and scale setting
#will compare results to grid point estimation to verify the match
######################################


##########################
#All i did was change the psi functions used in locAndScaleHuberAndProposal2.R
#Should be able to unify this all into one.
#####################################
...
#note: psi.tukey is available in the mass library: I write my ow because it gives psi(x)/x
fn.psi.tukey<-function(u, c = 4.685)
{
    return(u*(1 - pmin(1, abs(u/c))^2)^2)
}
x<-seq(-10,10, by=.001)
#plot(x,fn.psi.tukey(x), type='l', lwd=2)


fn.psi.tukey.prime<-function(u, c = 4.685){ #this is a strange form but it is correct
  t <- (u/c)^2
  ifelse(t < 1, (1 - t) * (1 - 5 * t), 0)
}
#plot(x,fn.psi.tukey.prime(x), type='l', lwd=2)


#needed for huber's proposal two.
...
fn.grads.tukey<-function(y, l1obs, s1obs, k=1.345){
  prime1<-fn.psi.tukey.prime((y-l1obs)/s1obs)
...
#computes y** from y*
fn.compute.ystst.tukey<-function(yst,l1obs,s1obs){
  fit.yst<-rlm(yst~1, psi=psi.bisquare, scale.est='Huber', maxit=400)
  l1yst<-coef(fit.yst)
  s1yst<-fit.yst$s
  res<-yst-l1yst
...
# fn.radius(ystst)
# yst<-rnorm(n)
# fityst<-rlm(yst~1,psi=psi.bisquare, scale.est='Huber')
# s1obs<-fityst$s
# l1obs<-coef(fityst)
# ystst<-fn.compute.ystst.tukey(yst, l1obs,s1obs)
# tst<-rlm(ystst~1,psi=psi.bisquare, scale.est='Huber')
# coef(tst)
# l1obs
# tst$s
# s1obs
...
fn.attenuation.tukey <- function(ystst, Vt=Vt, l1obs,s1obs)
{
  grads.ystst<-fn.grads.tukey(ystst, l1obs, s1obs, k=1.345)
...
fn.one.rep.y.tukey<- function(y.curr,mu,sig,l1obs, s1obs,n=length(y.curr))
{
  # propose new y vector
  y.prop <- rnorm(n)
  y.prop <- fn.compute.ystst.tukey(y.prop,l1obs, s1obs)  
  # compute acceptance ratio
  log.rat1 <- log.fn.eval.lik(y.prop,mu,sig)-log.fn.eval.lik(y.curr,mu,sig)
  
  log.prop.den.curr <-log(fn.attenuation.tukey(y.curr, Vt=Vt, l1obs,s1obs))-(n-2)*log(fn.radius(y.curr))
  log.prop.den.prop <-log(fn.attenuation.tukey(y.prop,Vt=Vt, l1obs,s1obs))-(n-2)*log(fn.radius(y.prop)) 
