...
# T(X), S(X)  LMS location and scale estimators
...
#####################
lms.obs<-lqs(newcomb~1, method='lms')
t.obs<-lms.obs$coefficients  #observed location estimate
s.obs<-lms.obs$scale[1]             #observed scale estimate, based on fit criterion
log.s.obs<-log(s.obs)
##############################################################
...
#########################
lms.estimators<-function(X){
  lms.out<-lqs(X~1, method='lms')
  return(c(as.numeric(lms.out$coefficients), log(as.numeric(lms.out$scale[1]))))
}
#############
...
lms.matrix<-apply(X,MARGIN=1, FUN=lms.estimators)
plot(lms.matrix[1,], lms.matrix[2,])
...
h1<-hpi(lms.matrix[1,], binned=TRUE)
h2<-hpi(lms.matrix[2,], binned=TRUE)
H<-diag(c(h1,h2))
...
sigma2.grid<-seq(5,40,length.out=ticks.sigma2)
...
#kernel density function using H as the bandwidth and lms.matrix as the data
kernel_density<-function(x1,x2){
  mean(dmvnorm(t(lms.matrix),mean=c(x1,x2),sigma=H^2))
}
...
# dev.new()
# pdf('marginaldistributionsLMS.pdf')
...
save.image(do.call(paste, list('wsNewcombDirectSamplingLMS',paramSet, '.RData', sep='')))
