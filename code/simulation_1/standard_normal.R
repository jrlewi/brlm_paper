# Data generating mechanism - Standard normal
...
write_rds(normal_post_means, here::here('out', 'normal_post_means.rds'))
...
write_rds(mixture_1_post_means, here::here('out', 'mixture_1_post_means.rds'))
...
write_rds(mixture_2_post_means, here::here('out', 'mixture_2_post_means.rds'))
...
write_rds(t_post_means, here::here('out', 't_post_means.rds'))

# fit restricted likelihood Tukey ----

X <- matrix(rep(1, length(y)), ncol = 1)
fit_tukey <- restrictedBayesLm(y, X, regEst = 'Tukey', mu0 = eta, Sigma0 = tau^2, a0 = a, b0 = b, sigma2Int = var(y),  nkeep = 2000, nburn = 1000)
tukey_post_means[j,i] <- mean(fit_tukey$mcmc[,1])
write_rds(tukey_post_means, here::here('out', 'tukey_post_means.rds'))


# fit restricted likelihood Huber ----

fit_huber <- restrictedBayesLm(y, X, regEst = 'Huber', mu0 = eta, Sigma0 = tau^2, a0 = a, b0 = b, sigma2Int = var(y),  nkeep = 2000, nburn = 1000)
huber_post_means[j,i] <- mean(fit_huber$mcmc[,1])
write_rds(huber_post_means, here::here('out', 'huber_post_means.rds'))

cat(paste(samp_sizes[i], j, '  at', Sys.time(), '\n'), file = here::here('out', 'log.txt'), append = TRUE)
  }
}
...
write_rds(means_all, here::here('out', 'means_all_wide.rds'))
means_all <- gather(means_all, n, mean, -'Fitted Model')
means_all
write_rds(means_all, here::here('out', 'means_all_long.rds'))