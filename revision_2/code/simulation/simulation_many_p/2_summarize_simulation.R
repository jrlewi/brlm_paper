...
gen_good_data <- function(true_value, n){
  #true_value - vector c(betas, sigma2)
  # n - number of samples to generate. 
  p <- length(true_value) - 1
...
predict_full_model <- function(estimates, gen_out, uncertainty = TRUE, model = 'Normal', nu = NULL){
...
sim_one_pred <- function(split_df, n, nu){
 ...
 preds <- predict_full_model(estimates, gen_out, uncertainty = TRUE, model = model, nu = nu) 
 cbind(y, preds)
}
...
ggplot(pmse, aes(prior_sd, pmse, group = method, col = method)) +
  geom_point(position = position_dodge(width = .75)) + 
